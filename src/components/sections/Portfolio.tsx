...
export default function Portfolio() {
  ...
  return (
    <section
      id="portfolio"
      className="py-24 relative bg-[#030711] border-t border-cyan-900/20"
    >
          ...
          {projects.map((pkg, idx) => (
            <FadeUp key={pkg.id} delay={idx * 0.1}>
              <a
                ...
                className={`${pkg.colSpan} ${pkg.rowSpan} min-h-[220px] md:min-h-0 rounded-3xl overflow-hidden relative border ${pkg.border} bg-gradient-to-br ${pkg.gradient} group cursor-pointer block transform transition-all duration-500 hover:scale-[1.02] hover:shadow-[0_0_40px_rgba(0,212,255,0.15)] flex flex-col justify-end p-8 h-full`}
              >
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex justify-between items-start">
                    <span className="text-[10px] font-black text-[#00D4FF] uppercase tracking-[0.2em] py-1.5 px-4 rounded-full bg-cyan-950/40 border border-[#00D4FF]/20 backdrop-blur-md shadow-sm">
                      {pkg.category}
                    </span>
                  ...
                  <div className="space-y-1">
                    <h3 className="text-2xl md:text-3xl font-bold text-white font-syne group-hover:text-[#00D4FF] transition-colors duration-300 flex items-center gap-3">
                      {pkg.title}
                    </h3>
                    <div className="h-0.5 w-12 bg-[#00D4FF]/40 rounded-full group-hover:w-20 transition-all duration-500" />
                  </div>
                </div>
              </a>
            </FadeUp>
